...
void sEE_LowLevel_DeInit(void)
{
	...
	/* Configure and enable I2C DMA RX Stream
	 * interrupt */
...
void sEE_LowLevel_Init(void)
{
	...
	/*!< sEE_I2C_SCL_GPIO_CLK and sEE_I2C_SDA_GPIO_CLK Periph clock
	 * enable */
	...
	/* Configure and enable I2C DMA TX
	 * Channel interrupt */
	NVIC_InitStructure.NVIC_IRQChannel = sEE_I2C_DMA_TX_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = sEE_I2C_DMA_PREPRIO;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = sEE_I2C_DMA_SUBPRIO;
...
{ 
	...
	{
		/* Configure the DMA Tx Stream with the buffer address and
		 * the buffer size */
	...
	{ 
		/* Configure the DMA Rx Stream with the buffer address
		 * and the buffer size */
		sEEDMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)pBuffer;
